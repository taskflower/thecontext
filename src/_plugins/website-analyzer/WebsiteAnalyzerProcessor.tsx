// src/plugins/website-analyzer/WebsiteAnalyzerProcessor.tsx
...
export const WebsiteAnalyzerProcessor: React.FC = memo(() => {
  ...
  // Get plugin options
  const options = currentNode.plugin === "website-analyzer" && currentNode.pluginOptions
    ? currentNode.pluginOptions["website-analyzer"] || {}
    : {};
  
  // Get plugin settings
  const buttonText = options.button_text || "Analyze Website";
  const userIdValue = user?.uid || "user123";
  const assistantMessage = options.assistant_message || "Enter a website URL to analyze its links.";
  ...
  const analyzeWebsite = async () => {
    ...
    try {
      ...
      const apiUrl = `${apiBaseUrl}/api/v1/services/analyze-website/links?url=${encodeURIComponent(websiteUrl)}`;
...
WebsiteAnalyzerProcessor.displayName = 'WebsiteAnalyzerProcessor';