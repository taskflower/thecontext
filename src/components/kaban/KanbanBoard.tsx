...
export const KanbanBoard: FC<{ instance: KanbanInstance }> = ({ instance }) => {
  ...
  const selectedTask = selectedTaskId
    ? instance.tasks.find((t) => t.id === selectedTaskId)
    : null;
  const selectedTemplateTask = selectedTask
    ? (template.tasks.find(
        (t) => t.id === selectedTask.templateTaskId
      ) as KanbanTaskTemplate)
    : null;

  // Sprawdzamy czy task ma wymagane pola
  if (
    selectedTemplateTask &&
    (!selectedTemplateTask.steps || selectedTemplateTask.steps.length === 0)
  ) {
    return (
      <Alert variant="destructive">
        <AlertDescription>
          Selected task doesn't have any steps defined. Please edit the template
          to add steps.
        </AlertDescription>
      </Alert>
    );
