...
const ConfigIndicator: React.FC<ConfigIndicatorProps> = ({ 
  ...
  const bgColor = isFirebase ? 'bg-blue-100' : 'bg-green-100';
  const textColor = isFirebase ? 'text-blue-800' : 'text-green-800';
  const borderColor = isFirebase ? 'border-blue-200' : 'border-green-200';
