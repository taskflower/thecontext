...
export class ContextValidator {
  ...
  private static validateLlmStep(
    ...
    if (schemaPath) {
      ...
      if (!schema) {
        issues.push(
          `Krok ${index + 1} (${
            step.label || "LLM"
          }): Nie znaleziono schematu LLM pod ścieżką ${schemaPath}`
        );
      }
    }
    ...
    if (step.attrs?.initialUserMessage) {
      ...
      templateVars.forEach((variable: any) => {
        ...
        if (value === undefined || value === null) {
          issues.push(
            `Krok ${index + 1} (${
              step.label || "LLM"
            }): Brakująca zmienna kontekstowa: ${path}`
          );
      ...
      if (
        ...
        issues.push(
          `Krok ${index + 1} (${
            step.label || "LLM"
          }): Auto-start nie zadziała z powodu brakujących zmiennych: ${missingVars.join(
            ", "
          )}`
        );
      }
    ...
    if (step.attrs?.autoStart === true) {
      ...
      if (
        ...
        step.attrs.initialUserMessage.trim() === ""
      ) {
        ...
        if (!webContext || !webContext.www) {
          issues.push(
            `Krok ${index + 1} (${
              step.label || "LLM"
            }): Auto-start wymaga kontekstu 'primaryWebAnalysing.www', który jest niedostępny`
          );
        }
      }
      ...
      if (step.contextPath) {
        ...
        if (stepData && Object.keys(stepData).length > 0) {
          issues.push(
            `Krok ${index + 1} (${
              step.label || "LLM"
            }): Auto-start może nie być potrzebny - dane już istnieją pod ścieżką ${
              step.contextPath
            }`
          );
        }
  ...
  private static validateFormStep(
    ...
    if (schemaPath) {
      ...
      if (!schema) {
        issues.push(
          `Krok ${index + 1} (${
            step.label || "Formularz"
          }): Nie znaleziono schematu formularza pod ścieżką ${schemaPath}`
        );
      }
    }
    ...
    if (step.contextPath) {
      ...
      if (contextValue && typeof contextValue === "object") {
        ...
        let formSchema: any[] = [];
        const possibleSchemaPath =
          step.attrs?.schemaPath || step.attrs?.formSchemaPath;
        ...
        if (formSchema.length > 0) {
          ...
          if (missingRequiredFields.length > 0) {
            issues.push(
              `Krok ${index + 1} (${
                step.label || "Formularz"
              }): Brakuje wartości dla wymaganych pól: ${missingRequiredFields.join(
                ", "
              )}`
            );
          }
  ...
  private static validateStepInputs(
      ...
      templateVars.forEach((variable: any) => {
        ...
        if (!contextPaths.has(rootPath)) {
          issues.push(
            `Krok ${index + 1} (${
              step.label || "Step"
            }): Wiadomość asystenta używa ścieżki "${path}", której część główna "${rootPath}" nie jest generowana przez żaden krok`
          );
        }
      });
    }
    ...
    if (!step.contextPath && !step.contextKey) {
      issues.push(
        `Krok ${index + 1} (${
          step.label || "Step"
        }): Brak zdefiniowanej ścieżki kontekstowej (contextPath lub contextKey)`
      );
    }
    ...
    if (step.templateId === "llm-query" && step.attrs?.autoStart === true) {
      ...
      if (index < flowSteps.length - 1) {
        ...
        if (nextStep.assistantMessage) {
          const rootContextKey = step.contextPath
            ? step.contextPath.split(".")[0]
            : step.contextKey;

          if (
            rootContextKey &&
            nextStep.assistantMessage.includes(`{{${rootContextKey}`)
          ) {
            issues.push(
              `Krok ${index + 2} (${
                nextStep.label || "Step"
              }): Ten krok może nie mieć dostępu do danych z poprzedniego kroku z auto-startem (${
                step.label || "LLM"
              })`
            );
          }
