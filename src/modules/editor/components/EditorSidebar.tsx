...
export default function EditorSidebar({ configName }: EditorSidebarProps) {
  ...
  const { workspace: currentWorkspace, scenario: currentScenario, type: currentType } = useParams();
  ...
  const loadWorkspacesAndScenarios = async () => {
    try {
      ...
      const potentialWorkspaces = [
        'main', 'tickets', 'users', 'profiles', 'dashboard', 
        'settings', 'reports', 'analytics', 'admin'
      ];

      for (const workspaceSlug of potentialWorkspaces) {
        try {
          const response = await fetch(`/src/_configs/${configName}/workspaces/${workspaceSlug}.json`);
  ...
  const loadScenariosForWorkspace = async (workspaceSlug: string): Promise<ScenarioInfo[]> => {
    const scenarios: ScenarioInfo[] = [];
    
    // Rozszerzona lista potencjalnych scenariuszy
    const potentialScenarios = [
      'list', 'create', 'edit', 'view', 'delete', 
      'llm-create', 'search', 'filter', 'export',
      'import', 'bulk-edit', 'archive', 'restore'
    ];

    for (const scenarioSlug of potentialScenarios) {
      try {
        const response = await fetch(`/src/_configs/${configName}/scenarios/${workspaceSlug}/${scenarioSlug}.json`);
        if (!response.ok) continue;
  ...
  const getScenarioPath = (workspaceSlug: string, scenarioSlug: string) => `/${workspaceSlug}/${scenarioSlug}`;
  ...
  const isCurrentScenario = (workspaceSlug: string, scenarioSlug: string) => {
    return currentWorkspace === workspaceSlug && currentScenario === scenarioSlug && currentType === 'scenario';
  };
  ...
  return (
    <div className="w-80 bg-zinc-50 border-r border-zinc-200 flex flex-col">
      ...
      <div className="p-4 border-b border-zinc-200">
        <h2 className="text-lg font-semibold text-zinc-900">📁 Configuration Explorer</h2>
                  ...
                  {workspace.scenarios.map((scenario) => (
                    <div key={scenario.slug} className="group">
                      <div className="flex items-center">
                        ...
                        <button
                          onClick={() => copyPath(getScenarioPath(workspace.slug, scenario.slug))}
                          ...
                          title={`Copy path: ${getScenarioPath(workspace.slug, scenario.slug)}`}
                        >
                          {copiedPath === getScenarioPath(workspace.slug, scenario.slug) ? (
                            <svg className="w-3 h-3 text-green-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
                            </svg>
                          ) : (
                          ...
                          {scenario.nodes
                            ...
                            .map((node) => (
                              ...
                              >
                                ...
                                <span className="truncate">{node.label || node.slug}</span>
      ...
      <div className="p-4 border-t border-zinc-200 space-y-2">
        ...
        <button
          onClick={() => navigate(`/${configName}/${appConfig?.defaultWorkspace || 'main'}`)}
