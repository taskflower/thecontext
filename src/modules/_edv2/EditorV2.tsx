...
export default function EditorV2({ isOpen, onClose }: EditorV2Props) {
  ...
  const scenarioConfigPath = `/src/_configs/${cfgName}/scenarios/${wsName}/${
  ...
  const saveConfig = async () => {
    ...
    try {
      const configPath =
        editMode === "scenario" && currentScenario
    ...
    <div className="fixed inset-y-0 left-0 w-96 bg-white border-r border-zinc-200 shadow-lg z-50 overflow-y-auto">
      ...
      {editConfig ? (
        <>
          <div className="p-4">
            {editMode === "scenario" ? (
              <ScenarioEditor config={editConfig} onChange={setEditConfig} />
            ) : (
              <WorkspaceEditor config={editConfig} onChange={setEditConfig} />
            )}
          </div>
          ...
          <div className="p-4 border-t border-zinc-200">
            ...
            >
              {saving
                ? "Saving..."
                : `Save ${editMode === "scenario" ? "Scenario" : "Workspace"}`}
            </button>
          </div>
        </>
      ) : (
