      ...
      if (workspaceIndex !== -1 && state.items[workspaceIndex].contextItems) {
        ...
        if (itemIndex !== -1) {
          if (payload.title !== undefined) {
            state.items[workspaceIndex].contextItems[itemIndex].title =
              payload.title;
          }

          if (payload.content !== undefined) {
            state.items[workspaceIndex].contextItems[itemIndex].content =
              payload.content;
          }

          state.items[workspaceIndex].contextItems[itemIndex].updatedAt =
            Date.now();
          state.items[workspaceIndex].updatedAt = Date.now();
          state.stateVersion++;
        }
