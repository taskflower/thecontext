...
export interface CollectionSchema {
  ...
  indexes?: {            // Indeksy dla szybszego wyszukiwania
    name: string;
    keyPath: string;
 ...
 */
export const initDatabase = async (config: DatabaseConfig): Promise<boolean> => {
  try {
    // 1. Sprawdź czy kontekst już istnieje
    const store = useAppStore.getState();
    const contextItems = store.getContextItems();
    const dbContext = contextItems.find(item => item.title === config.contextKey);
    ...
    return new Promise((resolve, reject) => {
      openRequest.onsuccess = () => {
        ...
        console.log(`Successfully opened database: ${config.dbName}, version: ${db.version}`);
        db.close();
        resolve(true);
      };
    ...
    return new Promise((resolve, reject) => {
      ...
      openRequest.onsuccess = () => {
        ...
        for (const item of items) {
          ...
          request.onerror = () => {
            console.error(`Error adding item to ${collectionName}:`, request.error);
            db.close();
        ...
        for (const id of ids) {
          ...
          request.onsuccess = () => {
            ...
            if (successCount === ids.length) {
              console.log(`Deleted ${successCount} items from ${collectionName}`);
              db.close();
              resolve(true);
            }
          };
          ...
          request.onerror = () => {
            console.error(`Error deleting item from ${collectionName}:`, request.error);
            db.close();
...
export const generateNodesFromData = (
  ...
  data.forEach((item: any, index: number) => {
    ...
    store.addNode({
      ...
      userPrompt: '',
      position: { x: currentX, y: currentY },
      pluginKey: template.pluginMapping ? template.pluginMapping(item) : undefined,
 ...
 */
export const createDatabaseConfig = (type: 'lessons' | 'surveys' | 'processes' | 'custom', options: any): DatabaseConfig => {
  switch (type) {
    case 'lessons':
      return {
