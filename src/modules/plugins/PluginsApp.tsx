...
const PluginsApp: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<"flow" | "dashboard" | "all">(
    "all"
  );
  ...
  const flowComponentKeys = useMemo(
    () => getPluginKeysByType("flow"),
    [getPluginKeysByType]
  );
  const dashboardComponentKeys = useMemo(
    () => getPluginKeysByType("dashboard"),
    [getPluginKeysByType]
  );
  ...
  useEffect(() => {
    if (visibleComponentKeys.length > 0 && !selectedComponent) {
      ...
      !visibleComponentKeys.includes(selectedComponent || "")
    ) {
      setSelectedComponent(visibleComponentKeys[0]);
    }
      ...
      <div className="border-b pb-4 pt-2 px-4">
        <div className="flex items-center justify-between">
          <Tabs
            value={activeTab}
            onValueChange={(value) =>
              setActiveTab(value as "flow" | "dashboard" | "all")
            }
            className="w-[400px]"
          >
            <TabsList>
      ...
      <div className="flex-1 overflow-hidden relative">
        {visibleComponentKeys.length === 0 ? (
          ...
          <div className="flex h-full ">
            ...
            <div className="w-3/5 p-4 overflow-auto sticky top-10">
