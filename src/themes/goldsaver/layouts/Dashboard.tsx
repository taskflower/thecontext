...
import { 
  Home, User, CreditCard, BarChart2, Archive, FileText, HelpCircle, 
  DollarSign, RefreshCw, AlertTriangle, LogOut, Settings, ChevronDown, Bell
} from 'lucide-react';
...
const DashboardLayout: React.FC<{ children: React.ReactNode; context?: any }> = ({ children, context }) => {
  ...
  const workspaceSlug = context?.workspace?.slug || location.pathname.split('/')[1] || '';
  ...
  const [goldPrice, setGoldPrice] = useState({ price: 300, change: 1.2 });
  ...
  return (
    <div className="flex min-h-screen bg-gray-50">
      ...
      <aside className="hidden lg:flex flex-col w-72 bg-white border-r border-gray-200 shadow-sm">
        <div className="px-6 py-5 border-b border-gray-200 flex items-center space-x-2">
        ...
        <div className="px-6 py-4 bg-yellow-50 border-b border-yellow-100">
          <div className="flex justify-between items-center">
            <div>
              ...
              <p className="text-lg font-bold text-yellow-700">{goldPrice.price} PLN</p>
            </div>
            <div className={`text-sm font-medium ${goldPrice.change >= 0 ? 'text-green-600' : 'text-red-600'}`}>
        ...
        <nav className="flex-1 py-6 px-3 space-y-1 overflow-y-auto">
          {menuItems.map(item => (
            ...
            >
              <span className={`mr-3 ${isActive(item.slug) ? 'text-yellow-600' : 'text-gray-500'}`}>
                {item.icon}
              </span>
          ...
          <div className="flex items-center justify-between px-4 py-3 lg:py-4">
            ...
            <div className="hidden lg:block">
              <h1 className="text-xl font-bold text-gray-900">
                {menuItems.find(item => isActive(item.slug))?.name || 'Dashboard'}
              </h1>
            </div>
                      ...
                      {notifications.length > 0 ? (
                        <div className="space-y-2">
                          {notifications.map(notification => (
                            <div 
                              key={notification.id}
                              className={`p-3 rounded-lg ${notification.isNew ? 'bg-blue-50' : 'bg-white hover:bg-gray-50'} transition-colors`}
                            >
        ...
        <footer className="mt-auto bg-white border-t border-gray-200 py-4 px-6 text-center text-sm text-gray-500">
          <div className="flex justify-between items-center max-w-7xl mx-auto">
            ...
            <div className="flex space-x-4">
              <a href="#" className="hover:text-gray-700">Regulamin</a>
              <a href="#" className="hover:text-gray-700">Polityka prywatności</a>
