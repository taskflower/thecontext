...
export default function ChatWidget() {
  const [messages, setMessages] = useState([
    { id: 1, sender: 'system', content: 'Witamy w czacie wsparcia. Jak możemy pomóc?', timestamp: new Date().toISOString() }
  ]);
  ...
  const handleSendMessage = () => {
    ...
    setTimeout(() => {
      const agentMessage = {
        ...
        sender: 'agent',
        content: 'Dziękujemy za wiadomość. Agent wsparcia odpowie najszybciej jak to możliwe.',
        timestamp: new Date().toISOString()
      };
              ...
              >
                {message.sender !== 'user' && (
                  <div className="flex items-center mb-1">
                    {message.sender === 'system' ? (
                      <p className="text-xs font-medium text-yellow-700">System</p>
                    ) : (
                      <p className="text-xs font-medium text-gray-500">Agent wsparcia</p>
                    )}
                  </div>
                )}
