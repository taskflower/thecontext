...
export default function FormStep({ attrs }: FormStepProps) {
  ...
  const { addItem, updateItem, getItem } = useDBData(attrs?.onSubmit?.collection || '');
  ...
  if (!schema) {
    return <div className="text-red-500">Schema not found: {attrs.schemaPath}</div>;
  }

  const { properties } = schema;
  const excludeFields = attrs.excludeFields || [];
  const fields = Object.entries(properties).filter(([key]) => !excludeFields.includes(key));
