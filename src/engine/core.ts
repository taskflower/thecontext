...
export const useWidgets = (widgets: Widget[] = [], templateDir?: string) => {
  const [loadedWidgets, setLoadedWidgets] = useState<Array<{ Component: ComponentType<any> } & Widget>>([]);
  ...
  useEffect(() => {
    ...
    Promise.all(
      widgets.map(async (widget) => {
        try {
          const module = await import(`/src/themes/${templateDir}/widgets/${widget.tplFile}`);
