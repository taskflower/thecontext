...
const NodeRenderer = memo(
  ({ config, node, onNext, stepIdx, totalSteps }: NodeRendererProps) => {
    ...
    const jsonSchema = useMemo(() => {
      ...
      const rootSchema =
        currentWorkspace.contextSchema.properties[rootProperty];
      if (!rootSchema) {
      ...
      for (let i = 1; i < pathSegments.length; i++) {
        ...
        if (
          ...
          currentLevel.properties[segment]
        ) {
          currentLevel = currentLevel.properties[segment];
        } else {
          console.warn(
            `Segment "${segment}" not found in schema at path "${path}"`
          );
