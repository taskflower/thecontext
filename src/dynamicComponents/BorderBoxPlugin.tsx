...
const SimpleBorderPlugin: React.FC<PluginComponentProps> = ({ data, appContext }) => {
  ...
  const handleWriteColorToUserMessage = () => {
    if (nodeId && appContext?.updateNodeUserPrompt) {
      ...
      setFeedback(`Cannot update: nodeId=${nodeId}, has function=${!!appContext?.updateNodeUserPrompt}`);
      setFeedbackType('error');
    }
  };
  ...
  const handleWriteColorToAssistantMessage = () => {
    if (nodeId && appContext?.updateNodeAssistantMessage) {
      try {
        ...
        const template = options.messageTemplate || 'I recommend using {color} as the border color.';
      ...
      setFeedback(`Cannot update: nodeId=${nodeId}, has function=${!!appContext?.updateNodeAssistantMessage}`);
      setFeedbackType('error');
    }
  };
    ...
    <div className="p-4">
      ...
      >
        ...
        <div className="flex-1 p-4 text-center">
          <p className="mb-4">This box has a border with color: <strong>{options.borderColor}</strong></p>
          
          {feedback && (
            <div className={`p-2 my-2 rounded-md text-sm ${
          ...
          <div className="p-2 bg-muted/20 rounded-md mb-4 text-xs text-left">
            ...
            <p>Assistant message: {appContext?.currentNode?.assistantMessage || '(empty)'}</p>
          </div>
        </div>
...
SimpleBorderPlugin.optionsSchema = {
  ...
  messageTemplate: {
    ...
    description: 'Template for assistant message. Use {color} to insert the color value.'
  }
};
