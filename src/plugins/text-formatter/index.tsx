...
// Nowy format pluginu zgodny z interfejsem Plugin
const formatterPlugin: Plugin = {
  ...
  process: async (message: string, options?: any): Promise<string> => {
    console.log('Formatter procesuje wiadomość');
    
    // Domyślna operacja
    const operation = options?.operation || 'capitalize';
...
// Dodaj funkcjonalność renderowania UI jako custom hook zamiast metody w obiekcie
// Można go później zaimportować w PluginOptions.tsx jeśli potrzeba

export const useFormatterOptionsUI = (options: any, onChange: (newOptions: any) => void) => {
  const operation = options.operation || 'capitalize';
