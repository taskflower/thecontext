...
const StatusCardWidget: PluginComponentWithSchema<StatusCardData> = ({
  data,
  appContext,
}) => {
  ...
  const numericValue =
    typeof displayValue === "number" ? displayValue : parseFloat(displayValue);
  const difference = !isNaN(numericValue)
    ? numericValue - widgetData.previousValue
    : 0;
  const percentChange =
    !isNaN(numericValue) && widgetData.previousValue
    ...
    <div className="p-4 h-full flex flex-col" style={containerStyle}>
      ...
      <div className="flex-1 flex items-center">
        <div className="text-3xl font-bold" style={valueStyle}>
          {displayValue}
          {widgetData.unit && (
            <span className="text-xl ml-1">{widgetData.unit}</span>
          )}
        </div>
      </div>
