...
const QuizInteractionTemplate: React.FC<FlowStepProps> = ({
  ...
  const questions = quizContent.pytania || [];
  ...
  const calculateScore = () => {
    ...
    const correctAnswers = questions.filter((q, index) => 
      userAnswers[index] === q.poprawna_odpowiedz
    ).length;
  ...
  const renderQuestion = () => {
    ...
    const isCorrect = showResults && userAnswer === question.poprawna_odpowiedz;
    const isIncorrect = showResults && userAnswer && userAnswer !== question.poprawna_odpowiedz;
    ...
    return (
      <div className="space-y-4">
        ...
        <h3 className="text-lg font-semibold">{question.tresc}</h3>
        
        <div className="space-y-2">
          {question.odpowiedzi && question.odpowiedzi.map((answer, idx) => (
            <div 
              key={idx}
              ...
              className={`p-3 rounded-lg cursor-pointer border ${
                userAnswer === answer 
                  ...
                  : showResults && answer === question.poprawna_odpowiedz
                    ? 'bg-green-100 border-green-300'
            ...
            >
              <div className="flex justify-between">
                ...
                {showResults && (
                  <span>
                    {answer === question.poprawna_odpowiedz && (
        ...
        {showResults && includeExplanations && question.wyjasnienie && (
          <div className="mt-4 p-3 bg-blue-50 rounded-lg border border-blue-200">
            <p className="font-medium">Wyjaśnienie:</p>
            <p>{question.wyjasnienie}</p>
          </div>
        )}
      </div>
    );
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-gradient-to-r from-blue-500 to-purple-600 p-4 rounded-lg text-white">
        <h2 className="text-xl font-bold">{quizContent.tytul_quizu || 'Quiz wiedzy'}</h2>
        <p className="text-sm opacity-80">
          Poziom trudności: {quizContent.poziom_trudnosci || 'dostosowany'}
        </p>
      </div>
