  ...
  private async loadFirebaseConfig(appId: string): Promise<AppConfig> {
    try {
      ...
      await Promise.all(
        scSnap.docs.map(async (scenarioDoc) => {
          ...
          try {
            ...
            console.log(
              `Znaleziono ${nodesSnap.size} węzłów dla scenariusza ${scenarioId}`
            );

            if (nodesSnap.empty) {
              console.warn(
                `Nie znaleziono węzłów dla scenariusza: ${scenarioId}`
              );
            ...
            const nodes = nodesSnap.docs
              .map((nd) => {
                ...
                return {
                  ...
                  saveToDB: ndat.saveToDB
                    ? parseIfString(ndat.saveToDB)
                    : undefined,
                };
              })
            ...
            console.error(
              `Błąd przy pobieraniu węzłów dla scenariusza ${scenarioId}:`,
              error
            );
            nodesMap.set(scenarioId, []);
          }
      ...
      const scenarios = scSnap.docs.map((d) => {
        ...
        console.log(
          `Scenariusz ${d.id} (${data.name}): ${nodes.length} węzłów`
        );
