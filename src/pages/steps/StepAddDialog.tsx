...
export function StepAddDialog({ taskId, open, onClose }: StepAddDialogProps) {
  ...
  const categorizedPlugins = Object.values(PLUGIN_CATEGORIES).reduce(
    (acc, category) => {
    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl">
        ...
        >
          ...
          {Object.entries(categorizedPlugins).map(
        ...
        {selectedPlugin && (
          <div className="space-y-2 mt-4">
            ...
            <p className="text-xs text-muted-foreground">
              This step will add a {selectedPlugin.name.toLowerCase()} to your
              workflow.
            </p>
          </div>
        )}
