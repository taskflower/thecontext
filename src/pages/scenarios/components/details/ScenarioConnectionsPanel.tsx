...
export const ScenarioConnectionsPanel: React.FC<ScenarioConnectionsPanelProps> = ({ 
  ...
  const { scenarios, getConnectedScenarios, removeScenarioConnection } = useDataStore();
  ...
  const getConnectionTypeBadge = (type?: ConnectionType) => {
    switch(type) {
      case 'dependency':
        return <Badge className="bg-amber-100 text-amber-800 border-amber-300">Depends On</Badge>;
      case 'parent':
        return <Badge className="bg-blue-100 text-blue-800 border-blue-300">Parent</Badge>;
      case 'child':
        return <Badge className="bg-green-100 text-green-800 border-green-300">Child</Badge>;
      case 'related':
      default:
        return <Badge className="bg-indigo-100 text-indigo-800 border-indigo-300">Related</Badge>;
    }
  };
    ...
    <>
      <Card>
        <CardHeader className="flex flex-row items-center justify-between pb-2">
          <div>
            <CardTitle className="text-xl font-bold">Connected Scenarios</CardTitle>
            <CardDescription>
              Link this scenario with others to create dependencies or relationships
            </CardDescription>
          </div>
          <Button 
        ...
        <CardContent>
          {connectedScenarios.length === 0 ? (
            <div className="text-center py-8 text-muted-foreground">
              ...
              <p className="text-sm">Connect this scenario with others to establish relationships</p>
            </div>
          ) : (
