...
export const DocumentList = () => {
  // Wywołujemy hook tylko raz na początku komponentu
  const { containers, getContainerDocuments } = useDocumentsStore();
              ...
              <TableBody>
                {containers.map((container) => (
                  <TableRow key={container.id}>
                    ...
                    <TableCell className="px-6">
                      {getContainerDocuments(container.id).length}
                    </TableCell>
                    <TableCell className="px-6">
                      <div className="flex space-x-2">
                        <Button
                          variant="outline"
                          size="sm"
                          onClick={() => navigate(`/admin/documents/${container.id}`)}
                        >
                          View Documents
                        </Button>
                        <Button
                          variant="outline"
                          size="sm"
                          onClick={() => navigate(`/admin/documents/${container.id}/edit`)}
                        >
                          Edit
                        </Button>
                      </div>
