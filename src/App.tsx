...
const AppWithConfig: React.FC = () => {
  const { config, loading, error, configType } = useConfig();
  ...
  return (
    <Router>
      <Routes>
        <Route path="/" element={<Navigate to={`/${config.workspaces[0].slug}`} replace />} />
        ...
        <Route path="*" element={<div className="p-4">Strona nie znaleziona</div>} />
      </Routes>
    </Router>
  );
};
